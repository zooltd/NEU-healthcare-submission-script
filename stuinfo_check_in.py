...
class Checkin:
    ...
    def _oauth_login(self, url1="http://stuinfo.neu.edu.cn/api/auth/oauth/"):
        url1 = (
            f"{url1}/token?username={self.stu_id}&grant_type=password&password={self.password}"
        ...
        print(f"Login fails: {access_info['message']}")
        print("Submission fails! Input 'Enter' to exit.")
        input()
        sys.exit()
    ...
    def _enter_check_in_page(
        self, url3="http://stuinfo.neu.edu.cn/cloud-xxbl/studentinfo?tag="
    ):
    ...
    def submit(
        ...
        if (r.status_code == 200) and (json.loads(r.text)["success"]):
            print("Submission success! Input 'Enter' to exit.")
        else:
            print("Submission fails! Input 'Enter' to exit.")
            print(r.status_code)
            print(r.text)
